...
namespace Otopark_Otomasyonu
{
    public partial class araccikis : Form
    {
        ...
        private void OtoBulAracPlaka()
        {
            baglanti.Open();
            OleDbCommand komut = new OleDbCommand("select * from musteri where durum='0'", baglanti);
            OleDbDataReader okuma = komut.ExecuteReader();
            while (okuma.Read())
            {
                comboBox1.Items.Add(okuma["plaka"].ToString());
            }
            baglanti.Close();
        }
        private void CikisTamamla()
        {
            if (comboBox1.Text == "")
            {
                mesaj.mesajlabeli = "Lütfen Plaka Girin.";
                mesaj.ShowDialog();
            }
            else
            {
        ...
        {
            baglanti.Open();
            OleDbCommand komut = new OleDbCommand("select * from musteri where durum='0' and plaka LIKE'" + comboBox1.Text + "'", baglanti);
            OleDbDataReader okuyucu = komut.ExecuteReader();
            while (okuyucu.Read())
            {
                boslabelmarka.Text = okuyucu["marka"].ToString();
                boslabelmodel.Text = okuyucu["model"].ToString();
                boslabelad.Text = okuyucu["ad"].ToString();
                boslabelsoyad.Text = okuyucu["soyad"].ToString();
                tarih = Convert.ToDateTime(okuyucu["gsaat"].ToString());
                parkyeri = okuyucu["park"].ToString();
            }
            baglanti.Close();
